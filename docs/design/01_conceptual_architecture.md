...
| 用語 (English) | 種類 | 定義 |
| :--- | :--- | :--- |
| **Document** | Noun | ユーザーから提供された入力ファイルやペイロード。永続化される前の一時的な状態。 |
| **Raw Record** | Noun | `Raw DB` に永続化されたデータ。元のコンテンツにシステム管理のメタデータが付与されたもの。 |
| **Ingest Metadata** | Noun | `Raw Record` に付随するメタデータ（追記日時、ソースのパスやURI、データ型など）。 |
| **Raw DB** | Noun | すべての `Raw Record` を格納する永続化ストア（例: SQLite）。システムの Single Source of Truth。 |
| **Ingest** | Verb | `Document` を受け取り、メタデータを付与して `Raw Record` として `Raw DB` に保存する行為。 |
...
| :------------ | :-------------- | :---------------------------- | :---------------- | :-------------------- |
| 外部からの入力ファイル   | **Document**    | (扱わない)                        | (扱わない)            | (扱わない)                |
...
C4Container
    ...
    System_Ext(dataSources, "Data Sources", "外部の生データ（ファイル、テキストストリーム等）。")
    ...
    Rel(dataSources, cli, "Document の提供", "File I/O")
