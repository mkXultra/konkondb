...
Pythonのループ内で `if record.created_at > last_build:` と書くと、毎回数百万件のレコードをメモリにロードする O(N) のコストがかかります。`raw_data.since()` を提供することで、内部の SQLite に対して `WHERE created_at > ?` を発行でき、極めて高速なインクリメンタルビルドが可能になります。
