...
5. **Raw DB は append-only である** — MVP では Raw Record の UPDATE/DELETE は行わない。データの修正が必要な場合は再 Ingest を行う
...
erDiagram
    raw_records {
        ...
        TEXT source_uri "NULL許容。データの出所"
        TEXT ingested_at "RFC3339 UTC 固定長。NOT NULL"
        TEXT content_type "NULL許容。MIME type等"
...
MVP の Raw DB 物理スキーマは、DX と境界の単純さを優先し、上記フィールドに **1対1で対応**させる。なお、将来 Raw DB に内部列（監査用・最適化用等）を追加することは許容される — `RawDataAccessor` が `RawRecord` に投影する限りプラグイン互換性は保たれる（セクション 2.2 参照）。

| `RawRecord` フィールド | SQLite 列名 | 型 | NULL | 説明 |
| :--- | :--- | :--- | :--- | :--- |
| `id` | `id` | `TEXT` | NOT NULL | Raw Record の安定識別子。ACL 境界では opaque `str` |
| `source_uri` | `source_uri` | `TEXT` | NULL 許可 | ファイルパス / URL / stdin 等の由来 |
| `ingested_at` | `ingested_at` | `TEXT` | NOT NULL | UTC 正規化された固定長 RFC3339 文字列 |
| `content_type` | `content_type` | `TEXT` | NULL 許可 | MIME type 等 |
...
### 5.3 `ingested_at` との関係

UUID v7 の先頭タイムスタンプと `ingested_at` は **論理的に同一時刻**から生成される。ただし `ingested_at` が正規のソートキーであり、UUID v7 の時間順序性は決定的 tie-break における副次的な利点として扱う。

> **注意:** `id` の辞書順ソートが厳密にingest順序を保証するとは限らない（UUID v7 の monotonic/non-monotonic は実装依存）。順序の正式な保証は `(ingested_at, id)` の複合キーによる。
...
`ingested_at` は SQLite の `TEXT` 型に以下の固定長形式で保存する。
...
| `since()` の `WHERE ingested_at > ?` | 固定長 UTC 文字列は辞書順比較で正しい時系列順になる |
...
CREATE TABLE IF NOT EXISTS raw_records (
    ...
    CHECK (content_type IS NULL OR content_type <> ''),

    -- Canonical RFC3339 UTC fixed-width: YYYY-MM-DDTHH:MM:SS.ffffffZ (27 chars)
    CHECK (length(ingested_at) = 27),
    CHECK (substr(ingested_at, 5, 1)  = '-'),
    CHECK (substr(ingested_at, 8, 1)  = '-'),
    CHECK (substr(ingested_at, 11, 1) = 'T'),
    CHECK (substr(ingested_at, 14, 1) = ':'),
    CHECK (substr(ingested_at, 17, 1) = ':'),
    CHECK (substr(ingested_at, 20, 1) = '.'),
    CHECK (substr(ingested_at, 27, 1) = 'Z')
) STRICT;
```

#### 列ごとの設計意図

- **`id`**: `TEXT PRIMARY KEY`（opaque な文字列 ID）。比較順序の再現性のため `COLLATE BINARY`。同一 `ingested_at` 内の決定的 tie-break に利用。
- **`source_uri`**: 由来不明（stdin 等）を表現するため NULL 許可。空文字は正規化せず `NULL` に統一（CHECK で空文字を拒否）。
- **`ingested_at`**: `since()` の比較キー。`ORDER BY ingested_at ASC, id ASC` の先頭キー。SQLite の暗黙 datetime 変換に依存せず、アプリで厳密に正規化して保存。
- **`content_type`**: 不明時は NULL。MIME 以外の識別子も許容（厳密な書式検証は行わない）。
- **`content`**: `RawRecord.content: str` に合わせ `TEXT NOT NULL`。JSON/CSV/TXT/Markdown 等は文字列として保存。

#### `STRICT` テーブル採用方針

- **採用理由:** SQLite における型ゆらぎ（例: `ingested_at` に数値が混入）を抑制し、ACL DTO (`RawRecord`) と物理表現の整合性を高める。
- **前提:** SQLite 3.37+（2021年11月リリース。2026年の新規プロジェクトでは十分安全な前提）。

### 7.2 インデックス設計

`02_interface_contracts.md` の順序契約:

- `__iter__()` の順序: `ORDER BY ingested_at ASC, id ASC`
- `since()` の順序: `ORDER BY ingested_at ASC, id ASC`
- `since()` のフィルタ: `ingested_at > timestamp`（exclusive）

これを満たすため、以下の複合インデックスを **必須**とする。

```sql
CREATE INDEX IF NOT EXISTS idx_raw_records_ingested_at_id
    ON raw_records (ingested_at ASC, id ASC);
```

**設計根拠:**

1. **WHERE 句**: `ingested_at > ?` のレンジスキャンにインデックスを使用。
2. **ORDER BY 句**: `ingested_at ASC, id ASC` がインデックスの順序と一致するため、追加のソートが不要（filesort 回避）。
...
書き込み性能と実装単純性を優先し、必須インデックスは `idx_raw_records_ingested_at_id` のみとする。
...
### 10.1 初期化 (Version 0 → 1)

データベースファイルが存在しない、または `PRAGMA user_version = 0` の場合、セクション 12 の DDL を実行して Version 1 のスキーマを作成する。
...
| :--- | :--- | :--- |
| `__iter__()` | `SELECT id, source_uri, ingested_at, content_type, content FROM raw_records ORDER BY ingested_at ASC, id ASC` | `idx_raw_records_ingested_at_id` |
| `__len__()` | `SELECT COUNT(*) FROM raw_records` | (内部最適化) |
| `since(ts)` → `__iter__()` | `SELECT ... FROM raw_records WHERE ingested_at > ? ORDER BY ingested_at ASC, id ASC` | `idx_raw_records_ingested_at_id` (レンジスキャン) |
| `since(ts)` → `__len__()` | `SELECT COUNT(*) FROM raw_records WHERE ingested_at > ?` | `idx_raw_records_ingested_at_id` |

### 11.1 行 → `RawRecord` 変換規約（読み取りパス）

`raw_records` の1行を `RawRecord` に変換する際の必須ルール:

- `ingested_at` (TEXT) → `RawRecord.ingested_at` (datetime): RFC3339 UTC 文字列をパースし、**必ず timezone-aware UTC の `datetime`** に変換する。naive `datetime` を生成してはならない。
- `source_uri` / `content_type` (NULL) → `None` にマッピングする。
...
4. `WHERE ingested_at > ?` にバインドして実行する。
5. 結果は必ず `ORDER BY ingested_at ASC, id ASC` で返す。
...
```sql
INSERT INTO raw_records (id, source_uri, ingested_at, content_type, content)
VALUES (?, ?, ?, ?, ?);
```

- `id`: システムが生成した UUID v7 文字列
- `ingested_at`: 正規化済み RFC3339 UTC 文字列（セクション 6.3 のルールに準拠）
- `source_uri` / `content_type`: 空文字は `NULL` に正規化する

---

## 12. 完全 DDL (Data Definition Language)

以下は Raw DB の初期スキーマ（Version 1）を構築する完全な SQL である。

```sql
-- =============================================================
-- konkon db — Raw DB Schema (Version 1)
-- =============================================================
...
単一の `ingested_at` だけでは同時刻衝突があり得るため、順序は必ず以下の複合キーで定義する:
...
`since()` は `ingested_at > timestamp` の **exclusive** 条件であるため、タイムスタンプ精度衝突時にプラグイン側のチェックポイント設計によっては取りこぼしが起こり得る。

これは `02_interface_contracts.md` の注意事項と一致する設計であり、本データモデルは以下を提供して将来拡張に備える:

- 決定的順序（`ingested_at`, `id`）
- 同順序に一致する複合インデックス

将来的に `(last_ingested_at, last_id)` ベースのカーソルを導入する場合も、同じインデックス設計を再利用できる。
...
- Raw Record の削除/アーカイブ機構（現状は append-only）
