...
| :--- | :--- | :--- |
| `__iter__()` | `SELECT id, ingested_at, content, meta FROM raw_records ORDER BY ingested_at ASC, id ASC` | `idx_raw_records_ingested_at_id` |
...
```sql
INSERT INTO raw_records (id, ingested_at, content, meta)
VALUES (?, ?, ?, ?);
```
