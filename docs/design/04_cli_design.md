...
設計の前提:
- `prd.md` のコマンド体系（init, ingest, build, search, serve）
...
```text
Developer ──▶ [konkon CLI] ──▶ Ingestion Context  (ingest)
...
| ストリーム | 用途 | 例 |
| :--- | :--- | :--- |
| **stdout** | **データ出力のみ**。パイプ・リダイレクト可能な構造化出力 | `search` の結果、`ingest` の Record ID |
...
`ingest`, `build`, `search` は `--format` オプションをサポートする。`build` は json モードでのみ stdout にステータス情報を出力する（text モードではサマリーは stderr に出力される）。`init` と `serve` は stdout にデータ出力を行わないため、`--format` が指定された場合は無視される（エラーとしない）。
...
**SIGINT の扱い:**
- **one-shot コマンド**（`init`, `ingest`, `build`, `search`）: シェル慣習に従い `128 + signal_number`（通常 `130`）が返る。CLI 側で明示的に `sys.exit(130)` する必要はない
...
└── .konkon/
    └── raw.db         # Raw DB（初回 ingest 時に遅延作成）
```
...
**どのコンテキストにも属さない（システムレベル）。** ファイルシステムへのテンプレート書き込みのみを行う。Raw DB の作成はこの時点では行わない（初回 `ingest` 時に遅延作成）。
...
def build(raw_data: RawDataAccessor) -> None:
    ...
    for record in raw_data:
        # record.id, record.content, record.ingested_at 等が利用可能
        pass
...
Raw DB ファイル（`.konkon/raw.db`）が存在しない場合、`ingest` コマンドの初回実行時に `03_data_model.md` セクション 12 の完全 DDL を実行してスキーマを自動作成する。DB 接続時には `03_data_model.md` で規定された PRAGMA（`journal_mode=WAL`, `busy_timeout=5000`, `foreign_keys=ON`, `synchronous=NORMAL`）を毎回適用し、`PRAGMA user_version` でスキーマバージョンを検証する。既知のバージョン差に対しては自動マイグレーションを適用する。未知のバージョン（CLI より新しいスキーマ等）の場合のみ終了コード `3` でエラーとする。
...
- `since()` のセマンティクス（exclusive: `ingested_at > timestamp`、順序: `ORDER BY ingested_at ASC, id ASC`）を変更しない
...
| `konkon ingest` | Ingestion Context | Raw Record の永続化 | **Write** | — |
...
| `ingest` | L1（Raw DB / Ingestion） | Raw Record 永続化 |
...
| `since()` の exclusive フィルタ | `build --since` が `ingested_at > timestamp` で実行される |
| `RawDataAccessor` の順序契約 | `ORDER BY ingested_at ASC, id ASC` を CLI / Accessor が保持 |
...
| 拡張 | 概要 |
| :--- | :--- |
| `konkon status` | Raw DB のレコード数、最終 ingest 日時、Context Store の状態を表示 |
